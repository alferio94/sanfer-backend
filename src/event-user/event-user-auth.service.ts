...
import { RefreshToken } from '../usuarios/entities/refresh-token.entity';
...
export class EventUserAuthService {
  ...
  async login(loginEventUserDto: LoginEventUserDto): Promise<{ user: Omit<EventUser, 'password'>; accessToken: string; refreshToken: string; }> {
    ...
    const isPasswordValid = await bcrypt.compare(loginEventUserDto.password, user.password);
  ...
  private async generateTokens(user: EventUser): Promise<{ accessToken: string; refreshToken: string; }> {
  ...
  async refreshTokens(refreshTokenDto: RefreshEventUserTokenDto): Promise<{ accessToken: string; refreshToken: string; }> {
